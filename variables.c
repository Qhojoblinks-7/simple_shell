...
/**
 * isChainDelimiter - Checks if the current character in the buffer
...
/**
 * checkChainStatus - Checks if we should continue chaining based on the
...
int replaceVariables(info_t *info)
{
	...
	for (i = 0; info->arguments[i]; i++)
	{
		...
		if (!stringCompare(info->arguments[i], "$?"))
		{
			replaceString(&(info->arguments[i]), stringDuplicate
					(convertNumber(info->status, 10, 0)));
			continue;
		}
		...
		{
			replaceString(&(info->arguments[i]), stringDuplicate
					(stringLocateChar(envNode->str, '=') + 1));
			continue;
		}
